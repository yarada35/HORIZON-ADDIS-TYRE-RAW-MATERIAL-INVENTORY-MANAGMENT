...
with tab2:
    ...
    existing_plan = st.session_state["annual_plan"].get(selected_month, {"days": 22, "targets": {}})
    with col2:
        working_days = st.number_input("2. Working Days", 0, 31, existing_plan["days"])
    
    plan_products = st.multiselect("3. Select Products to Plan", list(BOM_DATA.index), default=list(existing_plan["targets"].keys()))
    ...
    if plan_products:
        ...
        for i, p in enumerate(plan_products):
            with cols[i % 3]:
                target_inputs[p] = st.number_input(f"{p}", 0, 10000, existing_plan["targets"].get(p, 0), key=f"target_{p}")
...
with tab3:
    st.header("Inventory & Forecast Alarms")
    daily_cons = st.number_input("Avg Daily Consumption (KG):", 1.0, 10000.0, 100.0)
    df_alarms = INV_DF.copy()
    for days in [15, 30, 60, 90]:
        df_alarms[f"Req {days}d"] = daily_cons * days
        df_alarms[f"Alarm < {days}d"] = df_alarms["Ending"] < df_alarms[f"Req {days}d"]
    st.dataframe(df_alarms, use_container_width=True)

# --- TAB 4: ACTUAL VS PLANNED ---
with tab4:
    st.header("📉 Actual vs. Planned Performance")
    selected_month = st.selectbox("Select Month to Review", month_names, key="rev_month")
    
    if selected_month in st.session_state["annual_plan"]:
        plan = st.session_state["annual_plan"][selected_month]
        st.write(f"### Targets for {selected_month}")
        actuals = {}
        for prod, target in plan["targets"].items():
            actuals[prod] = st.number_input(f"Actual units for {prod}:", 0, 10000, target, key=f"act_{prod}")
        
        if st.button("Calculate Performance"):
            comparison = pd.DataFrame({"Target": plan["targets"], "Actual": actuals})
            comparison["Deviation"] = comparison["Actual"] - comparison["Target"]
            st.table(comparison.style.map(lambda v: 'color: red' if v > 0 else 'color: green', subset=["Deviation"]))
    else:
        st.warning("Please define a plan in the 'Monthly Planning' tab first.")

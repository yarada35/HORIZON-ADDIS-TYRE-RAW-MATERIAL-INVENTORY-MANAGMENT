...
# ----------------------------------------------------
# 📂 AUTOMATED FILE LOADER ENGINE (CACHE DISK CONTEXT)
# ----------------------------------------------------
# We change cache type to allow dynamic live changes without restarting the cloud container
...
def load_and_compile_factory_data():
    ...
    try:
        ...
        # Heavy sanitation step to strip invisible trailing data artifacts from spreadsheet software
...
# ----------------------------------------------------
# 📊 DEEP SANITATION PROFILE EXTRACTION ENGINE
# ----------------------------------------------------
raw_headers = list(df_cpd_tyre.columns)
tire_sizes_clean = []

for col in raw_headers:
    # Clean structural non-breaking spaces (\xa0) and newline items often generated by Excel CSV pipelines
    col_str = str(col).replace(r'\xa0', ' ').replace('\n', ' ').strip()
    col_str = ' '.join(col_str.split()) # Combines multiple nested blank blocks into single spaces
    
    if col_str != "Compound Type" and "Unnamed" not in col_str and col_str != "":
        tire_sizes_clean.append(col_str)

# Clean out item duplicates, sort cleanly by tire size grouping logic
...
with tab_dashboard:
    ...
    with col_input1:
        # Complete full dropdown loop display
    ...
    # COMPACT STABLE PROFILE FINDER
    if selected_size and selected_size != "No Data Found":
        matching_cols = [c for c in df_cpd_tyre.columns if str(c).strip() == str(selected_size)]
    ...
    for idx, row in df_planning.iterrows():
        ...
        calculated_add = base_add * scale_ratio * (raw_weight_value if raw_weight_value > 0 else 1.0)
...
# --- CACHE WIPE TRIGGER ENGINE ---
st.markdown("---")
if st.button("🗑️ Clear Cache Memory and Force Re-import All Tire Sizes"):
    st.cache_data.clear()
    st.success("Cache wiped! Streamlit is now scanning your new CSV dataset columns directly from GitHub.")
    st.rerun()

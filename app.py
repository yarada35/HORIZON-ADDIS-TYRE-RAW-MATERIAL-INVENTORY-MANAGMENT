...
def get_data():
    ...
    recipe_data = {
        ...
        "LN-2530": {"SMR-20 (SIR /SMR-20)": 0.55, "SBR 1500 (Kralex 1500)": 0.15, "NORMAL SULPHUR": 0.05},
...
with tab2:
    ...
    if plan_products:
        st.subheader(f"4. Define Daily Production Targets for {selected_month}")
        target_inputs = {}
        cols = st.columns(3)
        ...
        if st.button(f"Save Data for {selected_month}", type="primary"):
    ...
    if st.session_state["annual_plan"]:
        ...
        if report_data:
            ...
            annual_summary = df_final.groupby("Ingredient")["Total Required (KG)"].sum().reset_index()
            st.dataframe(annual_summary.style.format({"Total Required (KG)": "{:,.2f}"}), use_container_width=True)
